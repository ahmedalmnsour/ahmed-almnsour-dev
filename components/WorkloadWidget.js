...
export default function WorkloadWidget() {
  ...
  // 1. التعديل هنا: نبدأ من الصفر (0) بدلاً من رقم افتراضي
  const [bookedHours, setBookedHours] = useState(0); 
  
  // 2. التعديل هنا: الحالة المبدئية رمادية (جاري التحليل)
  const [systemStatus, setSystemStatus] = useState({ text: "جاري تحليل البيانات...", color: "#718096" });

  // 3. إضافة حالة للتأكد من التحميل (لمنع الوميض)
    ...
    <div className={styles.widgetContainer}>
      ...
      <div className={styles.header}>
        <div className={styles.statusIndicator}>
          ...
          الحالة: <span style={{ color: systemStatus.color, transition: 'color 0.5s ease' }}>{systemStatus.text}</span>
          </span>
        </div>
          ...
          {activeQueue.map((project) => (
            <div key={project.id} className={styles.queueItem}>
              <div className={styles.projectInfo}>
                <span style={{fontWeight: 'bold', color: project.type === 'private' ? '#A0AEC0' : '#63B3ED'}}>
                  {project.name}
                </span>
      ...
      <div className={styles.capacityBar}>
        <div style={{display:'flex', justifyContent:'space-between', marginBottom:'5px'}}>
           ...
           <span style={{color: systemStatus.color, fontWeight:'bold', transition: 'color 0.5s ease'}}>
             {/* عرض رقم 0 أثناء التحميل ثم الرقم الحقيقي */}
        ...
        <div className={styles.capacityTrack}>
          <div 
            ...
            style={{ 
                ...
                transition: 'width 1.5s ease-out, background 0.5s ease' // حركة انسيابية جميلة
            }}
          >
        ...
        <p style={{fontSize: '0.75rem', marginTop: '5px', color: '#718096', textAlign: 'right'}}>
